...
<Tabs>
  ...
  <Tab title="Windows ZIP">
    To install manually, download and install the Windows x86_64 release ZIP:
...
<Tabs>
  ...
  <Tab title="Windows ZIP">
    For a manual upgrade, download the latest
    `coral-x86_64-pc-windows-msvc.zip`, expand it, and overwrite the installed
    `coral.exe`:
